/* modules/memorama/memorama.js
   Memorama para Marina 2.

   Diseño:
   ┌─────────────────────────────────────────────────────────┐
   │ [🎨 Tema] · contador pares · [🔄 Reiniciar]            │ ← header mínimo
   ├─────────────────────────────────────────────────────────┤
   │                                                         │
   │   Grid 4×12 de cartas (48 cartas = 24 pares)           │
   │                                                         │
   ├─────────────────────────────────────────────────────────┤
   │ Tira de pares descubiertos (scroll horizontal)          │
   └─────────────────────────────────────────────────────────┘

   Interacción:
   · Al voltear carta → reproduce su nombre (MP3 → TTS)
   · Al encontrar par → nombre de 2ª carta + tono de victoria
   · Tira inferior → tocar un par reproduce su nombre
   · Selector de tema → modal cancelable
*/
...
// ─── Tono de victoria — beep generado por Web Audio ──────────────────────────
function _tonoVictoria() {
  try {
    ...
    notas.forEach((freq, i) => {
      ...
      gain.gain.linearRampToValueAtTime(0, ctx.currentTime + i * 0.12 + 0.18);
      osc.start(ctx.currentTime + i * 0.12);
      osc.stop(ctx.currentTime + i * 0.12 + 0.20);
    });
  } catch {}
}
...
let _cartas     = [];       // array de objetos carta
let _voltadas   = [];       // índices de cartas volteadas (máx 2)
let _descubiertas = new Set(); // índices de cartas ya pareadas
let _bloqueado  = false;    // evita toques durante animación
let _audioEl    = null;
let _lang       = 'es';
...
export async function init(container) {
  ...
  // Seleccionar primer tema automáticamente
...
function _render() {
  ...
  <style>
    ...
    #mm-btn-tema {
      ...
      padding: 8px 16px; border-radius: 99px; border: 1.5px solid rgba(255,255,255,0.25);
      background: rgba(255,255,255,0.12); color: #fff;
      font-family: inherit; font-weight: 800; font-size: 1rem;
      cursor: pointer; transition: background .15s, transform .12s;
      white-space: nowrap;
    }
    #mm-btn-tema:active { transform: scale(.94); background: rgba(255,255,255,.20); }
    #mm-tema-emoji { font-size: 1.2rem; }
    #mm-tema-label { max-width: 120px; overflow: hidden; text-overflow: ellipsis; }

    #mm-contador {
      flex: 1; text-align: center;
      font-size: 1rem; font-weight: 900; color: rgba(255,255,255,0.70);
      letter-spacing: .05em;
    }
    #mm-contador strong { color: #fff; font-size: 1.2rem; }

    #mm-btn-reiniciar {
      width: 40px; height: 40px; border-radius: 50%; border: 1.5px solid rgba(255,255,255,0.25);
      background: rgba(255,255,255,0.12); color: #fff;
    ...
    #mm-btn-reiniciar:active { transform: scale(.88) rotate(180deg); background: rgba(255,255,255,.22); }
    ...
    .mm-carta.bloqueada   { pointer-events: none; }
    ...
    .mm-carta-frente img {
      width: 70%; height: 55%; object-fit: contain;
    }
    .mm-carta-frente span {
      font-size: clamp(.5rem, 1.3vw, .78rem);
      font-weight: 800; color: #07212e; text-align: center;
      line-height: 1.1; word-break: break-word; padding: 0 2px;
    }

    /* ── Tira de descubiertos ── */
    #mm-tira-wrap {
    ...
    #mm-tira-placeholder {
      color: rgba(255,255,255,0.28); font-size: .85rem; font-weight: 600;
...
function _iniciarTema(tema) {
  ...
  _el.querySelector('#mm-victoria').classList.remove('visible');

  // Tomar exactamente PARES palabras (shuffle + slice)
  const palabras = _shuffle([...tema.palabras]).slice(0, PARES);

  // Crear 48 cartas (2 por palabra) y barajarlas
  _cartas = _shuffle(
  ...
  Telemetry.track('memorama_tema_iniciado', {
    _modulo: 'memorama', tema: tema.id, pares: PARES
  });
}
...
function _dibujarTablero() {
  ...
  _cartas.forEach((carta, idx) => {
    ...
    div.className = 'mm-carta' +
      (carta.volteada   ? ' volteada'    : '') +
      (carta.descubierta ? ' descubierta' : '');
    div.dataset.idx = idx;
  ...
  if (_voltadas.length === 2) {
    ...
    if (c1.id === c2.id) {
      ...
      setTimeout(() => {
        ...
        if (_descubiertas.size === _cartas.length) {
          setTimeout(_mostrarVictoria, 800);
        }
      }, 700);
    } else {
...
function _actualizarContador() {
  const pares = _descubiertas.size / 2;
  const el = _el.querySelector('#mm-pares-count');
  if (el) el.textContent = pares;
}
...
function _actualizarTira() {
  ...
  // Palabras únicas descubiertas (sin duplicados)
  const palabrasDesc = [...new Set(
    [..._descubiertas].map(i => _cartas[i].palabra)
  )];
  ...
  Object.entries(grupos).forEach(([grupo, temas]) => {
    ...
    temas.forEach(tema => {
      ...
      btn.addEventListener('click', () => {
        ...
        _el.querySelectorAll('.mm-tema-btn').forEach(b =>
...
function _abrirModal() {
  _renderModalTemas(); // refrescar activo
...
function _reproducirNombre(palabra) {
  ...
  const _fallback = () => {
    ...
    TTS.speak(palabra, { lang, rate: 0.90, pitch: 1.15 });
  };
