  ...
  _lista = _shuffle(ids.map(item => {
    if (typeof item === 'number') {
      ...
      return {
        picto: (_lang === 'en'
          ? (entrada.archivo_en || '')
          : (entrada.archivo_es || '')).replace('.png', ''),
        texto: _lang === 'en'
...
function _bindEvents() {
  ...
  _el.querySelector('#md-btn-escucha').addEventListener('click', () => {
    ...
    if (_lista.length) {
      const item = _lista[_idx];
      const texto = item.texto || item;
      const archivo = item.picto || item;   // ruta_img sin .png
      _hablar(texto, window.getLang?.() === 'en' ? 'en-US' : 'es-MX', archivo);
    }
  });
...
function _hablar(texto, lang = 'es-MX', nombreArchivo = null) {
  const langCode = lang.slice(0, 2);
  const archivoMp3 = nombreArchivo || texto;
  const url = `assets/audio/${langCode}/${archivoMp3}.mp3`;
