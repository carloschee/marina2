...
function bootMarina() {
  ...
  waitForGlobals(function () {
    loadConfig(function (config) {
      ...
      // Solicitar permiso de micrófono al arranque — una sola vez.
      // Safari recuerda la decisión para este origen permanentemente.
      // Así el diálogo no interrumpe dentro de ningún módulo.
      _solicitarMicrofono();
    });
  });
}

// Solicita acceso al micrófono silenciosamente tras el primer render.
// Si ya fue aceptado, retorna de inmediato sin mostrar diálogo.
function _solicitarMicrofono() {
  if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) return;
  navigator.mediaDevices.getUserMedia({ audio: true })
    .then(function (stream) {
      // Detener inmediatamente — solo necesitamos activar el permiso
      stream.getTracks().forEach(function (t) { t.stop(); });
    })
    .catch(function (e) {
      // Denegado o sin micrófono — no es error fatal
      console.warn('[marina] Micrófono no disponible:', e.message);
    });
}
